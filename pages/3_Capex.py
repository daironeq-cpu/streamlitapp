...
def tot_proj(df: pd.DataFrame, dados_tot: list[str]) -> pd.DataFrame:
    ...
    return (
        df
        .groupby((["num_projeto"] + dados_tot), as_index=False)
        .agg(
...
dados_tot = ["ano", "nom_obra_recurso", "dscntz_lct"]
...
dados_sel = st.sidebar.pills("Dados a Serem Totalizados", dados_tot, default=["ano"],selection_mode="multi")
...
dados_id_vars = ["num_projeto"] + dados_sel
...
df_total_mes = (
    df_graph
    .groupby("data", as_index=False)["capex"]
    .sum()
)

df_total_mes = df_total_mes.sort_values("data")
df_total_mes["capex_acum"] = df_total_mes["capex"].cumsum()

fig_graph = go.Figure()
fig_graph.add_trace(
    go.Bar(
        x=df_total_mes["data"],
        y=df_total_mes["capex"],
        name="CAPEX"
    )
)
fig_graph.add_trace(
